...
class RouteStepsForm extends StatefulWidget {
  ...
  final int stepIndex;
  const RouteStepsForm({super.key, required this.stepIndex, required this.stepName});
...
class _RouteStepsFormState extends State<RouteStepsForm> {
  ...
  Future<void> _loadSavedStep() async {
    ...
    if (widget.stepIndex < savedSteps.length && savedSteps[widget.stepIndex].isNotEmpty) {
      final savedData = savedSteps[widget.stepIndex];
      final Map<String, dynamic> stepMap = jsonDecode(savedData);

      setState(() {
        laborChargeCtrl.text = stepMap['laborCharge'] ?? '';
        workingTimeCtrl.text = stepMap['workingTime'] ?? '';
        materialEntries = List<Map<String, String>>.from(
          (stepMap['materials'] as List<dynamic>).map(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        ...
        child: SingleChildScrollView(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text("Section Labor Charge"),
              const SizedBox(height: 8),
              TextField(
                controller: laborChargeCtrl,
                keyboardType: TextInputType.number,
              ...
              const SizedBox(height: 16),
              const Text("Section Working Time"),
              const SizedBox(height: 8),
              TextField(
                controller: workingTimeCtrl,
                readOnly: true,
              ...
              const SizedBox(height: 16),
              const Text("Section Raw Material"),
              const SizedBox(height: 8),
              DropdownSearch<String>(
                popupProps: const PopupProps.menu(
                  showSearchBox: true,
      ...
      //     onPressed: () async {
      //       final prefs = await SharedPreferences.getInstance();
      //       final existingSteps = prefs.getStringList('savedRouteSteps') ?? [];
      //
      //       final stepData = {
      //         "stepName": widget.stepName,
      //         "laborCharge": laborChargeCtrl.text,      // can be empty
      //         "workingTime": workingTimeCtrl.text,      // can be empty
      //         "materials": materialEntries.map((e) => {
      //           "material": e['material'],
      //           "quantity": e['quantity'],
      //           "unit": e['unit'],
      //         }).toList(), // can be empty list
      //       };
      //
      //       // Ensure list length
      //       while (existingSteps.length <= widget.stepIndex) {
      //         existingSteps.add("");
      //       }
      //
      //       existingSteps[widget.stepIndex] = jsonEncode(stepData);
      //       await prefs.setStringList('savedRouteSteps', existingSteps);
      //
      //       ScaffoldMessenger.of(context).showSnackBar(
      //         const SnackBar(content: Text("Step saved locally")),
      ...
      bottomSheet: Container(
        ...
        child: ElevatedButton(
          onPressed: () async {
            // Check for partially entered material
            if ((selectedMaterial != null || quantityCtrl.text.isNotEmpty || selectedUnit != null) &&
                (selectedMaterial == null || quantityCtrl.text.isEmpty || selectedUnit == null)) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text("Please complete the material fields before saving")),
              );
              return;
            }
