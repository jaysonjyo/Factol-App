...
class _CreateProductPageState extends State<CreateProductPage> {
  ...
  Future<void> _clearLocalData() async {
    ...
    await prefs.remove('productActive');
    await prefs.remove('productSteps');
    await prefs.remove('savedRouteSteps');
  }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async => await _showExitConfirmationDialog(),
      child: Scaffold(
        appBar: AppBar(
          elevation: 0,
          backgroundColor: Colors.white,
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    Text("Steps", style: GoogleFonts.inter(fontWeight: FontWeight.bold, fontSize: 16)),
                    steps.isEmpty
                        ? Text("No steps found for this route", style: GoogleFonts.inter(color: Colors.grey))
                        : ListView.builder(
                      shrinkWrap: true,
                      physics: const NeverScrollableScrollPhysics(),
                      itemCount: steps.length,
                      itemBuilder: (ctx, i) => InkWell(
                        onTap: () async {
                          await Navigator.of(context).push(MaterialPageRoute(
                            builder: (_) => RouteStepsForm(stepIndex: i, stepName: steps[i],),
                          ));
                          setState(() {}); // Refresh to reflect any changes
                        },
                        child: ListTile(
                          title: Text(steps[i], style: GoogleFonts.inter(fontWeight: FontWeight.w600)),
                          subtitle: Text("Step ${i + 1}", style: GoogleFonts.inter(color: Colors.grey)),
              ...
              child: Row(
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      final prefs = await SharedPreferences.getInstance();
                      final savedSteps = prefs.getStringList('savedRouteSteps') ?? [];

                      // Build one step object per route step, merging any saved data
                      final List<Map<String, dynamic>> stepMaps = List.generate(steps.length, (i) {
                        // Defaults if the user didn't edit this step
                        ...
                        if (i < savedSteps.length && savedSteps[i].trim().isNotEmpty) {
                          try {
                            final decoded = jsonDecode(savedSteps[i]) as Map<String, dynamic>;
                            merged = {
                              "stepName": (decoded["stepName"] ?? steps[i]) as String,
                              "laborCharge": decoded["laborCharge"] ?? "",
                              "workingTime": decoded["workingTime"] ?? "",
                              "materials": (decoded["materials"] as List<dynamic>?)
                                  ?.map((m) => {
                                "material": (m as Map)["material"] ?? "",
                                "quantity": m["quantity"] ?? "",
                                "unit": m["unit"] ?? "",
                              })
                                  .toList() ??
                      ...
                      await docRef.set({
                        ...
                        "active": _isActive,
                        "route": _selectedRoute,
                        "steps": stepMaps.asMap().entries.map((e) => {
                          "order": e.key + 1,
                          "stepName": e.value["stepName"],
                          "laborCharge": e.value["laborCharge"],
                          "workingTime": e.value["workingTime"],
                          "materials": e.value["materials"],
                        }).toList(),
                        "createdAt": FieldValue.serverTimestamp(),
                      }, SetOptions(merge: true));
